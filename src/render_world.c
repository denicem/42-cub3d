...
/*   Updated: 2022/07/21 22:27:20 by mrojas-e         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_vertical_line(int ray_count, int wallStart, int wallEnd, t_data *data, t_ray *ray)
{
	...
	{
		if (i < wallStart)
			mlx_put_pixel(data->img, ray_count, i, get_rgba(38, 38, 38, 255));
		// if (i >= wallStart && i <= wallEnd)
		// 	mlx_put_pixel(img, ray, i, get_rgba(255, 0, 0, 128));
		if (i > wallEnd)
			mlx_put_pixel(data->img, ray_count, i, get_rgba(89, 34, 29, 150));
		i++;
	}
}
